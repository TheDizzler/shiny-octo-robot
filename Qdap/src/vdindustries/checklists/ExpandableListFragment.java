...
public class ExpandableListFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		final View view = inflater.inflate(R.layout.fragment_checklists_list, container, false);
		...
		expListView.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
				...
				String roomNo = listDataChild.get(listDataHeader.get(groupPosition)).get(childPosition);
	...
	public void onAttach(Activity activity) {
		...
		if (activity instanceof OnItemSelectedListener) {
			...
			throw new ClassCastException(activity.toString() + " must implemenet ExpandableListFragment"
	...
	private void prepareListData() {
		...
		for (int i = 0; i < floors.getLength(); ++i) {
			...
			String floorID = ((Element) floors.item(i)).getAttribute(Deficiency.FLOORID);

			List<String> roomList = new ArrayList<String>();
			roomList.addAll(DeficiencyParser.getRoomIDs(floorID));

			listDataChild.put(listDataHeader.get(i), roomList); // Header, Child
																// data
		}
	}
