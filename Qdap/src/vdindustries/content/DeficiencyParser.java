...
/**
 * A static class that holds the project data. Provides methods for reading and
 * writing to the project xml file, getting deficiencies by room and trade,
 * getting image files, various other convenient tasks.
 */
public class DeficiencyParser {
	...
	/**
	 * Internal true: Create using the DEFAULT project stored on internal
	 * storage. Internal false: Creates using the DEFAULT project stored in
	 * assets. Implementation: DeficiencyParser(getAssets())
	 */
	...
	/** Retrieves a list of all deficiencies in a room. */
	...
	/** Sets an image to the specified roomNo. */
	...
	private static void editDefic(Deficiency defic, Element reportNode) {
		...
		((Element) reportNode.getElementsByTagName("priority").item(0)).setTextContent((defic.priority) ? "true"
				: "false");
		((Element) reportNode.getElementsByTagName("coordinates").item(0)).setAttribute("x", Integer.toString(defic.X));
		((Element) reportNode.getElementsByTagName("coordinates").item(0)).setAttribute("y", Integer.toString(defic.Y));
