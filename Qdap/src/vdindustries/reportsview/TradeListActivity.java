...
/**
 * An activity representing a list of Trades. This activity has different
 * presentations for handset and tablet-size devices. On handsets, the activity
 * presents a list of items, which when touched, lead to a
 * {@link TradeDetailActivity} representing item details. On tablets, the
...
public class TradeListActivity extends ActionBarActivity implements TradeListFragment.Callbacks {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (findViewById(R.id.trade_detail_container) != null) {
			...
			((TradeListFragment) getSupportFragmentManager().findFragmentById(R.id.trade_list_fragment))
	...
	public void onItemSelected(String id) {
		...
		if (mTwoPane) {
			...
			getSupportFragmentManager().beginTransaction().replace(R.id.trade_detail_container, fragment).commit();
